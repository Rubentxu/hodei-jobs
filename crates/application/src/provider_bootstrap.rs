...
mod tests {
    ...
    impl ProviderConfigRepository for MockProviderConfigRepository {
        ...
        async fn find_by_id(&self, _id: &hodei_jobs_domain::shared_kernel::ProviderId) -> Result<Option<ProviderConfig>> {
            Ok(None)
        }
        ...
        async fn delete(&self, _id: &hodei_jobs_domain::shared_kernel::ProviderId) -> Result<()> {
            Ok(())
        }
