...
impl WorkerLifecycleManager {
    ...
    pub async fn run_health_check(&self) -> Result<HealthCheckResult> {
        let unhealthy = self.registry.find_unhealthy(self.config.heartbeat_timeout).await?;
    ...
    pub async fn cleanup_workers(&self) -> Result<CleanupResult> {
        ...
        for worker in workers_to_terminate {
            info!("Terminating worker {} (idle/lifetime exceeded)", worker.id());

            // Mark as terminating
            if let Err(e) = self.registry.update_state(worker.id(), WorkerState::Terminating).await {
                error!("Failed to mark worker {} as terminating: {}", worker.id(), e);
                continue;
            }
    ...
    pub async fn provision_worker(
        ...
        let provider = providers.get(provider_id).ok_or_else(|| DomainError::ProviderNotFound {
    ...
    async fn destroy_worker_via_provider(&self, worker: &Worker) -> Result<()> {
        let providers = self.providers.read().await;
        let provider = providers.get(worker.provider_id()).ok_or_else(|| {
    ...
    pub async fn should_scale_up(&self, pending_jobs: usize) -> bool {
        ...
        pending_jobs > self.config.scale_up_threshold && stats.total_workers < self.config.max_workers
    }
...
mod tests {
    ...
    impl WorkerRegistry for MockWorkerRegistry {
        async fn register(&self, handle: WorkerHandle, spec: WorkerSpec) -> Result<Worker> {
            ...
            self.workers.write().await.insert(handle.worker_id.clone(), worker.clone());
            Ok(worker)
        }
        ...
        async fn find(&self, _filter: &hodei_jobs_domain::worker_registry::WorkerFilter) -> Result<Vec<Worker>> {
            Ok(self.workers.read().await.values().cloned().collect())
        }

        async fn find_available(&self) -> Result<Vec<Worker>> {
            Ok(self.workers.read().await.values()
        ...
        async fn assign_to_job(&self, _worker_id: &WorkerId, _job_id: hodei_jobs_domain::shared_kernel::JobId) -> Result<()> {
            Ok(())
        }
