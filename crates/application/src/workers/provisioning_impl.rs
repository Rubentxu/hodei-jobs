...
use hodei_jobs_domain::iam::WorkerBootstrapTokenStore;
...
impl WorkerProvisioningService for DefaultWorkerProvisioningService {
    async fn provision_worker(
        ...
        if !matches!(
            health,
            hodei_jobs_domain::workers::HealthStatus::Healthy
                | hodei_jobs_domain::workers::HealthStatus::Degraded { .. }
        ) {
    ...
    async fn is_provider_available(&self, provider_id: &ProviderId) -> Result<bool> {
        ...
        match provider.health_check().await {
            Ok(hodei_jobs_domain::workers::HealthStatus::Healthy) => Ok(true),
            Ok(hodei_jobs_domain::workers::HealthStatus::Degraded { reason }) => {
