    ...
    async fn build_scheduling_context(&self, job: Job) -> Result<SchedulingContext> {
        ...
        for (provider_id, provider) in providers.iter() {
            ...
            let health_score = match health {
                Ok(hodei_jobs_domain::workers::HealthStatus::Healthy) => 1.0,
                Ok(hodei_jobs_domain::workers::HealthStatus::Degraded { .. }) => 0.5,
                _ => 0.0,
            };
...
mod tests {
    ...
    impl WorkerRegistry for MockWorkerRegistry {
        async fn register(
            &self,
            _handle: hodei_jobs_domain::workers::WorkerHandle,
            _spec: WorkerSpec,
        ...
        async fn find(
            &self,
            _filter: &hodei_jobs_domain::workers::WorkerFilter,
