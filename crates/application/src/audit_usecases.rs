...
mod tests {
    ...
    impl AuditRepository for MockAuditRepository {
        ...
        async fn find_by_event_type(
            ...
            limit: i64,
            offset: i64,
        ) -> Result<hodei_jobs_domain::audit::AuditQueryResult> {
            let logs = self.saved_logs.lock().unwrap();
        ...
        async fn count_by_event_type(&self) -> Result<Vec<hodei_jobs_domain::audit::EventTypeCount>> {
            let logs = self.saved_logs.lock().unwrap();
            ...
            Ok(counts
                .into_iter()
                .map(|(event_type, count)| hodei_jobs_domain::audit::EventTypeCount { event_type, count })
                .collect())
        }
