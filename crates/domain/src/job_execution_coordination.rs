...
use crate::job_execution::{Job, JobRepository, ExecutionContext};
...
pub trait LogRetriever: Send + Sync {
    ...
    async fn get_logs_since(&self, job_id: &JobId, since: DateTime<Utc>) -> Result<Vec<JobLogEntry>>;
...
impl LogAggregator {
    ...
    pub async fn aggregate_multiple_job_logs(&self, job_ids: &[JobId]) -> Result<Vec<AggregatedExecutionResult>> {
    ...
    fn calculate_execution_metrics(&self, job: &Job, logs: &[JobLogEntry]) -> ExecutionMetrics {
        ...
        let avg_cpu_usage = logs
            ...
            .sum::<f32>() / (logs.len() as f32).max(1.0);

        // Obtener tipo de provider
        let provider_type = job.selected_provider
            .as_ref()
            .map(|id| id.to_string())
            .unwrap_or_else(|| "Unknown".to_string());

        // Estimar costo
        let estimated_cost = job.selected_provider
            .as_ref()
