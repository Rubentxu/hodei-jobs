...
pub mod test_in_memory {
    ...
    use hodei_jobs_domain::jobs::{Job, JobQueue, JobRepository};
    ...
    pub struct InMemoryWorkerRegistry {
        workers: Arc<
            RwLock<
                HashMap<
                    hodei_jobs_domain::shared_kernel::WorkerId,
                    hodei_jobs_domain::workers::Worker,
                >,
            >,
        >,
    }
    ...
    #[async_trait]
    impl hodei_jobs_domain::workers::registry::WorkerRegistry for InMemoryWorkerRegistry {
        async fn register(
            &self,
            handle: hodei_jobs_domain::workers::WorkerHandle,
            spec: hodei_jobs_domain::workers::WorkerSpec,
        ...
        async fn find(
            &self,
            filter: &hodei_jobs_domain::workers::registry::WorkerFilter,
        ...
        async fn find_available(&self) -> Result<Vec<hodei_jobs_domain::workers::Worker>> {
            ...
            Ok(workers
                .values()
                .filter(|w| *w.state() == hodei_jobs_domain::shared_kernel::WorkerState::Ready)
                .cloned()
                .collect())
        }
        ...
        async fn update_state(
            &self,
            worker_id: &hodei_jobs_domain::shared_kernel::WorkerId,
            state: hodei_jobs_domain::shared_kernel::WorkerState,
        ) -> Result<()> {
            let mut workers = self.workers.write().await;
            if let Some(worker) = workers.get_mut(worker_id) {
                // Simplified state update for tests
                match state {
                    hodei_jobs_domain::shared_kernel::WorkerState::Creating => {
                        // Start in Creating
                    }
                    hodei_jobs_domain::shared_kernel::WorkerState::Connecting => {
                        let _ = worker.mark_connecting();
                    }
                    hodei_jobs_domain::shared_kernel::WorkerState::Ready => {
                        let _ = worker.mark_ready();
                    }
                    hodei_jobs_domain::shared_kernel::WorkerState::Busy => {
                        // Simulate job assignment to create Busy state
                        let _ = worker.assign_job(hodei_jobs_domain::shared_kernel::JobId::new());
                    }
                    hodei_jobs_domain::shared_kernel::WorkerState::Draining => {
                        let _ = worker.mark_draining();
                    }
                    hodei_jobs_domain::shared_kernel::WorkerState::Terminating => {
                        let _ = worker.mark_terminating();
                    }
                    hodei_jobs_domain::shared_kernel::WorkerState::Terminated => {
                        let _ = worker.mark_terminated();
                    }
                };
            }
            Ok(())
        ...
        async fn stats(&self) -> Result<hodei_jobs_domain::workers::registry::WorkerRegistryStats> {
            let workers = self.workers.read().await;
            Ok(hodei_jobs_domain::workers::registry::WorkerRegistryStats {
    ...
    pub struct InMemoryProviderConfigRepository {
        configs: Arc<
            RwLock<
                HashMap<
                    hodei_jobs_domain::shared_kernel::ProviderId,
                    hodei_jobs_domain::providers::ProviderConfig,
                >,
            >,
        >,
    }
    ...
    #[async_trait]
    impl hodei_jobs_domain::providers::ProviderConfigRepository for InMemoryProviderConfigRepository {
        async fn save(&self, config: &hodei_jobs_domain::providers::ProviderConfig) -> Result<()> {
        ...
        async fn find_by_id(
            &self,
            id: &hodei_jobs_domain::shared_kernel::ProviderId,
        ) -> Result<Option<hodei_jobs_domain::providers::ProviderConfig>> {
        ...
        async fn find_by_type(
            &self,
            provider_type: &hodei_jobs_domain::workers::ProviderType,
        ) -> Result<Vec<hodei_jobs_domain::providers::ProviderConfig>> {
            let configs = self.configs.read().await;
            Ok(configs
                .values()
        ...
        async fn find_enabled(&self) -> Result<Vec<hodei_jobs_domain::providers::ProviderConfig>> {
            let configs = self.configs.read().await;
            Ok(configs
                .values()
        ...
        async fn find_all(&self) -> Result<Vec<hodei_jobs_domain::providers::ProviderConfig>> {
            let configs = self.configs.read().await;
        ...
        async fn update(
            &self,
            config: &hodei_jobs_domain::providers::ProviderConfig,
        ) -> Result<()> {
            let mut configs = self.configs.write().await;
            configs.insert(config.id.clone(), config.clone());
            Ok(())
        ...
        async fn delete(&self, id: &hodei_jobs_domain::shared_kernel::ProviderId) -> Result<()> {
            let mut configs = self.configs.write().await;
            configs.remove(id);
            Ok(())
        }
