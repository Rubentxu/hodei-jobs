...
use hodei_jobs_domain::shared_kernel::DomainError;
use sqlx::postgres::PgPoolOptions;
use sqlx::{Pool, Postgres};

use async_trait::async_trait;
use hodei_jobs_domain::iam::{OtpToken, WorkerBootstrapTokenStore};
...
impl WorkerBootstrapTokenStore for PostgresWorkerBootstrapTokenStore {
    ...
    async fn consume(&self, token: &OtpToken, worker_id: &WorkerId) -> Result<()> {
        ...
        .await
        .map_err(
            |e| hodei_jobs_domain::shared_kernel::DomainError::InfrastructureError {
        ...
        if rows_affected == 0 {
            return Err(
                hodei_jobs_domain::shared_kernel::DomainError::InvalidOtpToken {
