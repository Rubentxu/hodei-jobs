use hodei_jobs_domain::{
    shared_kernel::{WorkerId, ProviderId, WorkerState},
    worker::{WorkerSpec, WorkerHandle, ProviderType},
...
async fn test_postgres_worker_lifecycle() {
    ...
    // 2. Register
    let worker = registry.register(handle, spec).await.expect("Failed to register worker");
    assert_eq!(*worker.id(), worker_id);
    assert_eq!(*worker.state(), WorkerState::Creating);

    // 3. Get by ID
    let found = registry.get(&worker_id).await.expect("Failed to get worker");
    assert!(found.is_some());
    let found_worker = found.unwrap();
    assert_eq!(*found_worker.id(), worker_id);

    // 4. Update State (Connecting -> Ready)
    registry.update_state(&worker_id, WorkerState::Connecting).await.expect("Failed to update state");
    let w = registry.get(&worker_id).await.expect("Get failed").unwrap();
    assert_eq!(*w.state(), WorkerState::Connecting);

    registry.update_state(&worker_id, WorkerState::Ready).await.expect("Failed to update state");
    let w = registry.get(&worker_id).await.expect("Get failed").unwrap();
    ...
    registry.heartbeat(&worker_id).await.expect("Failed to heartbeat");
    let w_after = registry.get(&worker_id).await.expect("Get failed").unwrap();
    assert!(w_after.last_heartbeat() > before_heartbeat);

    // 6. Find Available
    let available = registry.find_available().await.expect("Failed to find available");
    assert!(available.iter().any(|w| *w.id() == worker_id));

    // 7. Unregister
    registry.unregister(&worker_id).await.expect("Failed to unregister");
    let found_deleted = registry.get(&worker_id).await.expect("Failed to get deleted");
