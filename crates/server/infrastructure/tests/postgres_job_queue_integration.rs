use hodei_server_domain::{
    jobs::{Job, JobQueue, JobRepository, JobSpec},
    shared_kernel::JobId,
};
use hodei_server_infrastructure::persistence::{
...
async fn test_postgres_job_queue_fifo() {
    ...
    let job1 = Job::new(
        job1_id.clone(),
        JobSpec::new(vec!["echo".to_string(), "one".to_string()]),
    );
    ...
    let job2 = Job::new(
        job2_id.clone(),
        JobSpec::new(vec!["echo".to_string(), "two".to_string()]),
    );
