...
use hodei_server_application::jobs::cancel::{CancelJobResponse, CancelJobUseCase};
use hodei_server_application::jobs::create::{
    CreateJobRequest, CreateJobResponse, CreateJobUseCase,
};
use hodei_server_application::jobs::queries::{GetJobStatusUseCase, TrackJobResponse};
use hodei_server_application::providers::ProviderRegistry;
use hodei_server_application::providers::usecases::{
...
#[async_trait::async_trait]
impl RegisterProviderUseCaseTrait for hodei_server_application::providers::RegisterProviderUseCase {
    async fn execute(
        &self,
        request: RegisterProviderRequest,
    ) -> Result<RegisterProviderResponse, hodei_server_domain::shared_kernel::DomainError> {
        self.execute(request).await
    }
}

#[async_trait::async_trait]
impl ListProvidersUseCaseTrait for hodei_server_application::providers::ListProvidersUseCase {
    async fn execute(
        &self,
        request: ListProvidersRequest,
    ) -> Result<ListProvidersResponse, hodei_server_domain::shared_kernel::DomainError> {
        self.execute(request).await
    }
}

#[async_trait::async_trait]
impl GetProviderUseCaseTrait for hodei_server_application::providers::GetProviderUseCase {
    async fn execute(
        &self,
        provider_id: hodei_server_domain::shared_kernel::ProviderId,
    ) -> Result<GetProviderResponse, hodei_server_domain::shared_kernel::DomainError> {
...
pub trait GetAuditLogsUseCaseTrait: Send + Sync {
    async fn execute(
        ...
        Vec<hodei_server_domain::audit::AuditLog>,
        hodei_server_domain::shared_kernel::DomainError,
    >;
}

#[async_trait::async_trait]
impl GetAuditLogsUseCaseTrait for hodei_server_application::audit::AuditService {
    async fn execute(
        &self,
        correlation_id: String,
    ) -> Result<
...
fn parse_job_id(job_id: &str) -> Result<hodei_server_domain::shared_kernel::JobId, StatusCode> {
...
fn parse_provider_id(
    provider_id: &str,
) -> Result<hodei_server_domain::shared_kernel::ProviderId, StatusCode> {
    let uuid = uuid::Uuid::parse_str(provider_id).map_err(|_| StatusCode::BAD_REQUEST)?;
    Ok(hodei_server_domain::shared_kernel::ProviderId(uuid))
}
...
async fn create_job(
    ...
    let app_request = CreateJobRequest {
        spec: hodei_server_application::jobs::create::JobSpecRequest {
...
) -> Result<AxumJson<ApiResponse<Vec<hodei_server_domain::audit::AuditLog>>>, StatusCode> {
    match state
...
async fn create_app_state_from_env() -> anyhow::Result<AppState> {
    ...
    let provider_repository = std::sync::Arc::new(provider_repository)
        as std::sync::Arc<dyn hodei_server_domain::providers::ProviderConfigRepository>;
    ...
    let job_repository = std::sync::Arc::new(job_repository)
        as std::sync::Arc<dyn hodei_server_domain::jobs::JobRepository>;
    let job_queue =
    ...
    let audit_service =
        hodei_server_application::audit::AuditService::new(std::sync::Arc::new(audit_repository));
...
#[async_trait::async_trait]
impl CreateJobUseCaseTrait for hodei_server_application::jobs::create::CreateJobUseCase {
    async fn execute(
        &self,
        request: CreateJobRequest,
    ) -> Result<CreateJobResponse, hodei_server_domain::shared_kernel::DomainError> {
...
#[async_trait::async_trait]
impl GetJobStatusUseCaseTrait for hodei_server_application::jobs::queries::GetJobStatusUseCase {
    async fn execute(
        &self,
        job_id: hodei_server_domain::shared_kernel::JobId,
    ) -> Result<TrackJobResponse, hodei_server_domain::shared_kernel::DomainError> {
...
#[async_trait::async_trait]
impl CancelJobUseCaseTrait for hodei_server_application::jobs::cancel::CancelJobUseCase {
    async fn execute(
        &self,
        job_id: hodei_server_domain::shared_kernel::JobId,
    ) -> Result<CancelJobResponse, hodei_server_domain::shared_kernel::DomainError> {
...
mod tests {
    ...
    struct MockEventBusWithAudit {
        ...
        audit_logs: Arc<Mutex<Vec<hodei_server_domain::audit::AuditLog>>>,
    }
    impl MockEventBusWithAudit {
        fn new(audit_logs: Arc<Mutex<Vec<hodei_server_domain::audit::AuditLog>>>) -> Self {
            Self {
    ...
    impl EventBus for MockEventBusWithAudit {
        async fn publish(&self, event: &DomainEvent) -> std::result::Result<(), EventBusError> {
            ...
            let audit_log = hodei_server_domain::audit::AuditLog::new(
    ...
    fn create_app_state_for_tests() -> AppState {
        let job_repository = std::sync::Arc::new(
            hodei_server_infrastructure::repositories::InMemoryJobRepository::new(),
        )
            as std::sync::Arc<dyn hodei_server_domain::jobs::JobRepository>;
        let job_queue =
            std::sync::Arc::new(hodei_server_infrastructure::repositories::InMemoryJobQueue::new())
                as std::sync::Arc<dyn hodei_server_domain::jobs::JobQueue>;

        let provider_repo =
            hodei_server_infrastructure::repositories::in_memory::InMemoryProviderConfigRepository::new();
        ...
        AppState {
            ...
            get_audit_logs_usecase: std::sync::Arc::new(
                hodei_server_application::audit::AuditService::new(std::sync::Arc::new(
    ...
    struct MockAuditRepository {
        pub saved_logs: Arc<Mutex<Vec<hodei_server_domain::audit::AuditLog>>>,
    }

    impl MockAuditRepository {
        fn new_with_logs(logs: Arc<Mutex<Vec<hodei_server_domain::audit::AuditLog>>>) -> Self {
            Self { saved_logs: logs }
        }
    }

    #[async_trait::async_trait]
    impl hodei_server_domain::audit::AuditRepository for MockAuditRepository {
        async fn save(
            &self,
            log: &hodei_server_domain::audit::AuditLog,
        ...
        async fn find_by_event_type(
            ...
            hodei_server_domain::audit::AuditQueryResult,
            hodei_server_domain::shared_kernel::DomainError,
        > {
            let logs = self.saved_logs.lock().unwrap();
        ...
        async fn count_by_event_type(
            ...
            Vec<hodei_server_domain::audit::EventTypeCount>,
            hodei_server_domain::shared_kernel::DomainError,
        > {
            let logs = self.saved_logs.lock().unwrap();
    ...
    async fn test_get_audit_logs() {
        ...
        let api_response: ApiResponse<Vec<hodei_server_domain::audit::AuditLog>> =
    ...
    async fn test_create_job() {
        ...
        let audit_bytes = audit_response
            ...
            .unwrap()
            .to_bytes();
        let audit_logs: ApiResponse<Vec<hodei_server_domain::audit::AuditLog>> =
