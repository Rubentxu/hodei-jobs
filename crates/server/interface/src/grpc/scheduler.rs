...
pub struct SchedulerServiceImpl {
    ...
    job_queue: Arc<dyn hodei_server_domain::jobs::JobQueue>,
...
impl SchedulerService for SchedulerServiceImpl {
    async fn schedule_job(
        ...
        match decision {
            ...
            hodei_server_domain::scheduling::SchedulingDecision::Enqueue { reason, .. } => {
                job.metadata_mut()
    ...
    async fn configure_scheduler(
        ...
        if let Some(v) = plugin_cfg.get("worker_strategy") {
            scheduler_config.worker_strategy = match v.as_str() {
                "first_available" => {
                    hodei_server_domain::scheduling::WorkerSelectionStrategy::FirstAvailable
                }
                "least_loaded" => {
                    hodei_server_domain::scheduling::WorkerSelectionStrategy::LeastLoaded
                }
                "round_robin" => {
                    hodei_server_domain::scheduling::WorkerSelectionStrategy::RoundRobin
                }
                "most_capacity" => {
                    hodei_server_domain::scheduling::WorkerSelectionStrategy::MostCapacity
                }
                "affinity" => hodei_server_domain::scheduling::WorkerSelectionStrategy::Affinity,
