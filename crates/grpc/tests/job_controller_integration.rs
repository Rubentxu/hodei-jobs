...
use hodei_jobs_infrastructure::repositories::{InMemoryJobQueue, InMemoryJobRepository, InMemoryWorkerRegistry};
...
async fn job_controller_dispatches_run_job_to_connected_worker() {
    ...
    let job_queue = Arc::new(InMemoryJobQueue::new()) as Arc<dyn hodei_jobs_domain::job_execution::JobQueue>;
    ...
    // OTP and register
    let otp = worker_service.generate_otp(&worker_uuid.to_string()).await.expect("generate_otp should succeed");
