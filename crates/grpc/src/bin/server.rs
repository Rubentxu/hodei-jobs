...
use hodei_jobs_grpc::services::{
    ...
    LogStreamService, LogStreamServiceGrpc,
};
...
use hodei_jobs_infrastructure::providers::{
    ...
    FirecrackerConfig, FirecrackerProvider,
};
...
use hodei_jobs_infrastructure::persistence::{
    ...
    PostgresWorkerBootstrapTokenStore,
};
use hodei_jobs::{
...
impl GrpcDatabaseSettings {
    fn from_env() -> anyhow::Result<Self> {
        let url = env::var("HODEI_DATABASE_URL")
            ...
            .map_err(|_| anyhow::anyhow!("Missing database url (HODEI_DATABASE_URL or DATABASE_URL)"))?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let provisioning_enabled = env::var("HODEI_PROVISIONING_ENABLED")
    ...
    let scheduler_service = if provisioning_enabled {
        ...
        let docker_enabled = env::var("HODEI_DOCKER_ENABLED")
            .unwrap_or_else(|_| "1".to_string())
            == "1";
        ...
        if docker_enabled {
            match DockerProvider::new().await {
                Ok(provider) => {
                    ...
                    providers.insert(provider_id, std::sync::Arc::new(provider) as std::sync::Arc<dyn WorkerProvider>);
                }
                Err(e) => {
        ...
        let k8s_enabled = env::var("HODEI_K8S_ENABLED")
        ...
        if k8s_enabled {
            ...
            match KubernetesProvider::with_config(k8s_config).await {
                Ok(provider) => {
                    info!("  ✓ Kubernetes provider initialized (namespace: {})", 
                        env::var("HODEI_K8S_NAMESPACE").unwrap_or_else(|_| "hodei-workers".to_string()));
                    let provider_id = provider.provider_id().clone();
                    providers.insert(provider_id, std::sync::Arc::new(provider) as std::sync::Arc<dyn WorkerProvider>);
                }
                Err(e) => {
        ...
        let fc_enabled = env::var("HODEI_FC_ENABLED")
            .unwrap_or_else(|_| "0".to_string())
            == "1";
        ...
        if fc_enabled {
            ...
            match FirecrackerProvider::with_config(fc_config).await {
                Ok(provider) => {
                    info!("  ✓ Firecracker provider initialized (data_dir: {})",
                        env::var("HODEI_FC_DATA_DIR").unwrap_or_else(|_| "/var/lib/hodei/firecracker".to_string()));
                    let provider_id = provider.provider_id().clone();
                    providers.insert(provider_id, std::sync::Arc::new(provider) as std::sync::Arc<dyn WorkerProvider>);
                }
                Err(e) => {
        ...
        if !providers.is_empty() {
            ...
            let server_address = format!("http://{}:{}", 
                env::var("HODEI_SERVER_HOST").unwrap_or_else(|_| "host.docker.internal".to_string()),
                port
            );
            let provisioning_config = ProvisioningConfig::new(server_address)
                .with_default_image(
                    env::var("HODEI_WORKER_IMAGE").unwrap_or_else(|_| "hodei-worker:latest".to_string())
                );
            ...
            let provisioning_service = std::sync::Arc::new(DefaultWorkerProvisioningService::new(
                ...
                providers,
                provisioning_config,
            )) as std::sync::Arc<dyn hodei_jobs_application::worker_provisioning::WorkerProvisioningService>;
    ...
    let controller_enabled = env::var("HODEI_JOB_CONTROLLER_ENABLED")
        .unwrap_or_else(|_| "1".to_string())
        == "1";
    ...
    if controller_enabled {
        ...
        let sender = std::sync::Arc::new(GrpcWorkerCommandSender::new(worker_service_for_controller))
            as std::sync::Arc<dyn hodei_jobs_application::worker_command_sender::WorkerCommandSender>;
