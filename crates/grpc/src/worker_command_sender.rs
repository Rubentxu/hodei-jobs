...
use hodei_jobs::{
    command_spec::CommandType as ProtoCommandType, ArtifactInput, ArtifactOutput, CommandSpec,
...
impl GrpcWorkerCommandSender {
    ...
    fn build_run_job_command(job: &Job) -> RunJobCommand {
        let cmd = match &job.spec.command {
            ...
            CommandType::Script { interpreter, content } => CommandSpec {
    ...
    async fn send_to_worker(&self, worker_id: &WorkerId, cmd: RunJobCommand) -> std::result::Result<(), Status> {
...
impl WorkerCommandSender for GrpcWorkerCommandSender {
    async fn send_run_job(&self, worker_id: &WorkerId, job: &Job) -> Result<()> {
        ...
        self.send_to_worker(worker_id, cmd)
            .await
            .map_err(|e| hodei_jobs_domain::shared_kernel::DomainError::InfrastructureError {
                message: format!("Failed to send RunJobCommand to worker {}: {}", worker_id, e),
            })?;
