...
echo "🚀 Hodei Jobs Server - Dev Mode"
echo "================================"

# Wait for DevSpace to sync code (if not already present)
wait_for_code() {
    echo "⏳ Waiting for source code to be synced by DevSpace..."
    ...
    if [ -f "$SOURCE_DIR/Cargo.toml" ]; then
        ...
        echo "⚠️  No source code found. DevSpace will sync it."
    fi
}
...
compile_if_needed() {
    if [ -f "$SERVER_BIN" ]; then
        echo "✅ Binary already exists at $SERVER_BIN"
        return 0
    fi

    if [ ! -f "$SOURCE_DIR/Cargo.toml" ]; then
        echo "❌ No source code found. Run: devspace dev"
        echo "   DevSpace will sync the code and you can compile manually."
        exit 1
    fi

    echo "🔨 Compiling in pod..."
    cd "$SOURCE_DIR"
